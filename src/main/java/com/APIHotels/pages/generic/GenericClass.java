...
public class GenericClass extends LocalDriverManager {
	...
	/*
	 * String airlinesUrl = "http://10.10.103.152:3080/ACESAIR/"; String aces2Url =
	 * "http://10.10.103.152:3005/ACES/";
	 */
	...
	public int findOrEditRoomBlockRequest(String locationValue, String adhocType, String checkInDateValue,
		...
		pgWrapper.findOrEditRoomBlockRequestsPage.findOrEditRoomBlockRequests(locationValue, checkInDateValue,
				checkOutDateValue);
	...
	public String createRequestReservation(String destinationValue, String timeFormatValue, String arrivaltimeValue,
			String departuretimeValue, String arrivalFlightCodeValue, String arrivalFlightNumberValue,
			String departureFlightCodeValue, String departureFlightNumberValue, String additionalEmailAddressValue,
			String hotel, String singleRoomType, String doubleRoomType, String reasonCodeValue) throws IOException {
		pgWrapper = LocalDriverManager.getPageWrapper();
		pgWrapper.requestReservationPage.requestReservationPage(destinationValue, timeFormatValue, arrivaltimeValue,
				departuretimeValue, arrivalFlightCodeValue, arrivalFlightNumberValue, departureFlightCodeValue,
				departureFlightNumberValue, additionalEmailAddressValue);
	...
	/*
	 * Connects to SFTP and copies the file from local machine to specified location
	 * in Winscp
	 */
	public void uploadFileToSFTP(String sftpFolderPath, String scenarioFolderName) throws Exception {
		...
		File directory = new File(autoprocessingFilesPath + File.separator + scenarioFolderName + File.separator);
	...
	public void verifylogsInPutty(String sftpFolderPath, String command, String keyWordInLogs) throws Exception {
		...
		while (true) {
			while (in.available() > 0) {
				...
				ExtentTestManager.getTest().log(LogStatus.INFO, "The Log " + logs);
...
//				 channel.getExitStatus());
	...
	/*
	 * This method moves files from Local to SSH location and Process Files inside
	 * the scenarioFolderName based on tenantName
	 */
	...
	public void processPairingFilesInSSH(String tenantName, String scenarioFolderName) throws Exception {
		...
		if (tenantName.equalsIgnoreCase("JetBlue")) {
			uploadFileToSFTP(readPropValue("sftpPath_JetBlue"), scenarioFolderName);
			...
			uploadFileToSFTP(readPropValue("sftpPath_Latam"), scenarioFolderName);
	...
	/*
	 * This Method call processPairingFilesInSSH and process add_Pairing_1.xml file.
	 * Books the trips from opsDesk-> Dashboard and verifies the status in Find Trip
	 * page
	 */
	...
	/*
	 * This Method call processPairingFilesInSSH and process add_pairing_2.xml file.
	 * Verifies the status and changes in trips by clicking on show Notes link in
	 * Find Trip page
	 */
...
//		pgWrapper.pageFindTrip.verifySupplierAndStatus(hotelName, status);
//		// pgWrapper.pageFindTrip.clickOnShowNotesLink();
//		pgWrapper.pageFindTrip.verifyTripsNotes(textToVerifyInNotes);
	}
	...
	public void processPairingFilesInSSHRoomBlock(String tenantName, String scenarioFolderName) throws Exception {
		...
		System.out.println("executing processPairingFilesInSSH");
		if (tenantName.equalsIgnoreCase("JetBlue")) {
			uploadFileToSFTP(readPropValue("sftpPath_JetBlue_RoomBlock"), scenarioFolderName);
...
//				getDriver().get(acesLatamUrl);
	...
	public void verifyAddPairing2BookAvailableRes(String sshIndicator, String scenarioFolderName, String tenantName,
			String reservationId, String checkInDateValue, String hotelName, String confirmationCode, String status,
			String scenario, String processType, String textToVerifyInNotes) throws Exception {
//		if (sshIndicator.equals("Yes")) {
//			pgWrapper.genericClass.processPairingFilesInSSH(tenantName, scenarioFolderName);
//		}
		ExtentTestManager.getTest().log(LogStatus.INFO, "Executing scenario " + scenario);
...
//				getDriver().get(acesLatamUrl);
		...
		pgWrapper.pagePendingConfirmations.enterConfirmationCode(confirmationCode);
		pgWrapper.pagePendingConfirmations.clickNextButton();
		pgWrapper.pagePendingConfirmations.verifyErrorHeaderText("Number of riders changed from 1 to 2");
		pgWrapper.pagePendingConfirmations.clickFinishButton();
		ExtentTestManager.getTest().log(LogStatus.INFO, "Completed Booking PA " + reservationId);
		pgWrapper.opsDeskMenu.clickFindTripLink();
		pgWrapper.pageFindTrip.findTrips(reservationId, checkInDateValue);
	...
	public void bookAddPairing1TripsIgnoringBothGT(String sshIndicator, String scenarioFolderName, String tenantName,
			String reservationId, String checkInDateValue, String hotelName, String confirmationCode,
			String hotelRateValue, String paymentMethod, String status, String supplierType, String scenario)
			throws Exception {
		pgWrapper = LocalDriverManager.getPageWrapper();
		...
		pgWrapper.pageDashBoard.ignorebothGTs();
		pgWrapper.pagePendingConfirmations.clickNextButton();
		pgWrapper.pagePendingConfirmations.addHotelRate(hotelRateValue);
		pgWrapper.pagePendingConfirmations.addToHotelGTRates("12", "13");
		pgWrapper.pageDashBoard.selectPaymentMethod(paymentMethod);
		...
		while (true) {
			while (in.available() > 0) {
				...
				if (logs.contains(keyWord)) {
					ExtentTestManager.getTest().log(LogStatus.INFO, "The Keyword " + keyWord + " is found in logs ");
					System.out.println("The Keyword you are searching is found in log");
					channel.disconnect();
					break;
				} else if (logs.contains("FAILURE")) {
					ExtentTestManager.getTest().log(LogStatus.INFO, "The Keyword failure is found in logs ");
					Assert.fail("Failed to process the file in sftp");
				}
				Thread.sleep(3000);
			}
	...
	public void ProcessOpsPairingFilesInSSH(String tenantName, String scenarioFolderName, String keyword)
			throws Exception {
		System.out.println("Executing processPairingFilesInSSH");
		if (tenantName.equalsIgnoreCase("Air France")) {
			ExtentTestManager.getTest().log(LogStatus.INFO, "Processing of Ops File Started");
			uploadFileToSFTP(integrationAFFilePath, readPropValue("sftpOpsPath_AirFrance"), scenarioFolderName);
			connectToSftpAndExecuteCommand("tail -25f /home/aces2dev/acesII/logs/tripCompareServerConsole.*", keyword);
			ExtentTestManager.getTest().log(LogStatus.INFO, "Processing of Ops File Completed");
		}
	}

	public void uploadFileToSFTP(String MainFolderName, String sftpFolderPath, String scenarioFolderName)
			throws Exception {
		String[] sftpHostValue = sftpHost.split(",");
		String sftpHost = sftpHostValue[0];
		String sftpHost_Latam = sftpHostValue[1];
	...
	public void verifylogsInPutty(String sftpFolderPath, String command, String keyWordInLogs, String keyWord2InLogs) throws Exception {
		String[] sftpHostValue = sftpHost.split(",");
		String sftpHost_JetBlue = sftpHostValue[0];
		String sftpHost_Latam = sftpHostValue[1];
		...
		while (true) {
			while (in.available() > 0) {
				...
				if (logs.contains(keyWordInLogs) && logs.contains(keyWord2InLogs)) {
					ExtentTestManager.getTest().log(LogStatus.INFO,
							"The Keyword " + keyWordInLogs + " "+keyWord2InLogs+" is found in logs ");
					System.out.println("The Keyword you are searching is found in log " + keyWordInLogs +" for "+ keyWord2InLogs);
					channel.disconnect();
					break;
				} else if (logs.contains("BUILD FAILED") || logs.contains("Error")) {
					ExtentTestManager.getTest().log(LogStatus.INFO,
